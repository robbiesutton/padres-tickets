...
function loadReport(): PlaywrightReport | null {
  ...
  if (!fs.existsSync(p)) {
    console.warn('No results.json found');
    return null;
  }
...
function flattenResults(suites: Suite[]): FlatResult[] {
  ...
  function walk(suite: Suite) {
    for (const spec of suite.specs || []) {
      ...
      const flaky =
        anyPassed && tests.some((t) => t.results && t.results.length > 1);
      const status: FlatResult['status'] = anyFailed
        ? 'failed'
        : flaky
          ? 'flaky'
          : anyPassed
      ...
      const attachments =
        failedResult?.results?.flatMap((r) => r.attachments || []) || [];
      // 'image-diff' is the visual regression diff image; fall back to 'screenshot'
      const screenshot = attachments.find(
        (a) => (a.name === 'image-diff' || a.name === 'screenshot') && a.path
      );
      ...
      results.push({
        ...
        errorMessage: failedResult?.results?.[0]?.errors?.[0]?.message?.slice(
          0,
          300
        ),
...
3. Fix the underlying app issue (not the test)`;
  ...
  try {
    const queryRes = await fetch(
      `https://api.notion.com/v1/databases/${tasksDbId}/query`,
      {
        method: 'POST',
        headers: {
    ...
    if (existing.results.length > 0) {
      ...
      await notion.comments.create({
        ...
        rich_text: [
          {
            text: {
              content: `Reproduced on PR #${prNumber} (${commitSha.slice(0, 7)}) — ${new Date().toISOString().slice(0, 10)}${screenshotUrl ? `\nScreenshot: ${screenshotUrl}` : ''}`,
            },
          },
        ],
      });
...
async function main() {
  ...
  if (total === 0) {
    console.log('No tests ran — skipping reporter.');
    process.exit(0);
  }
  ...
  if (process.env.VERCEL_BLOB_RW_TOKEN) {
    ...
    ({ reportUrl, screenshotUrls } = await uploadArtifacts(
      prNumber,
      commitSha
    ));
    await pruneOldBlobs();
  ...
  const matrixRows = results
    .map((r) => {
      const screenshot = r.screenshotPath
        ? screenshotUrls[r.screenshotPath]
        : undefined;
  ...
  if (notion && verificationDbId) {
    ...
    if (tasksDbId && failures.length > 0) {
      console.log(
        `Processing ${failures.length} failure(s) for Notion tasks...`
      );
      for (const failure of failures) {
        const screenshotUrl = failure.screenshotPath
          ? screenshotUrls[failure.screenshotPath]
          : undefined;
        await upsertFailureTask(
          notion,
          tasksDbId,
          failure,
          prNumber,
  ...
  if (githubToken && githubRepo && prNumber !== 'N/A') {
    const lines = [
      ...
      ...(notionPageUrl
        ? [`📋 [Notion Verification Page](${notionPageUrl})`]
        : []),
    ];
