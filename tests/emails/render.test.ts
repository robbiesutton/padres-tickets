...
// LAUNCH_CHECKLIST: All email templates render correctly.
// These tests verify every template produces valid HTML with no undefined interpolations.
...
describe('Email templates', () => {
  describe('auth-email — magic link', () => {
    it('renders without errors', () => {
      const result = buildMagicLinkEmail(
        'Mark',
        'https://example.com/magic?token=abc123'
      );
  ...
  describe('claim-confirmation', () => {
    ...
    it('renders correctly for free game (pricePerTicket = null)', () => {
      const result = buildClaimConfirmationEmail({
        ...baseData,
        pricePerTicket: null,
      });
      expect(result.html).toBeTruthy();
  ...
  describe('transfer-action', () => {
    const baseData = {
      ...
      markTransferredUrl:
        'https://benchbuddy.app/api/games/xyz/mark-transferred?token=abc',
    };
    ...
    it('renders correctly for free game (pricePerTicket = null)', () => {
      const result = buildTransferActionEmail({
        ...baseData,
        pricePerTicket: null,
      });
      expect(result.html).toBeTruthy();
  ...
  describe('game-day — claimer reminder', () => {
    ...
    it('renders without row (general admission)', () => {
      const result = buildClaimerGameDayEmail({
        ...baseData,
        row: null,
