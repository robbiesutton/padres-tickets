...
import { useState, useEffect, useRef, startTransition, createContext, useContext } from 'react';
...
function SeatInfoPillDropdown({ pkg, isDark, navColor, teamAccent, holderFirstName, onPkgUpdate }: {
  pkg: PackageForNav;
  isDark: boolean;
  navColor: string;
  teamAccent: string;
  ...
  const [seatPhotoUrl, setSeatPhotoUrl] = useState<string | null>(pkg.seatPhotoUrl);
  ...
  async function handleSave() {
    ...
    const [pkgRes, userRes] = await Promise.all([
      ...
      fetch('/api/users/me', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ venmoHandle: trimmedVenmo, zelleInfo: trimmedZelle }),
      }),
    ]);
        ...
        <div className="p-[22px]">
          ...
          {editing ? (
              ...
              <div className="grid grid-cols-[1.1fr_1fr] gap-6 items-stretch">
                ...
                <div className="flex flex-col gap-[18px]">
                  ...
                  <div className="opacity-50 pointer-events-none select-none">
                    ...
                    <p className="text-[11px] text-[#8e8985] mt-1.5 italic">Set during setup. Not editable here.</p>
                  </div>
            ...
            <div className="grid grid-cols-[1.3fr_1fr] gap-[22px] items-center">
              ...
              >
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#8e8985" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="opacity-55 mb-3">
                  <rect x="3" y="3" width="18" height="18" rx="2" />
                  <circle cx="8.5" cy="8.5" r="1.5" />
                  <path d="M21 15l-5-5L5 21" />
                </svg>
                <div className="text-[15px] font-semibold text-[#1B1716] mb-1">Add a seat photo</div>
                <div className="text-xs text-[#8e8985]">Claimers will see this on your share page</div>
              </div>
              <div className="text-center px-[18px] py-6">
                ...
                <div className="text-[15px] font-bold text-[#1B1716] mb-1">Add your seat details</div>
...
export default function DashboardLayout({
  ...
  const userInitial =
    holderFirstName.charAt(0).toUpperCase() || '';
