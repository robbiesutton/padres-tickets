...
const MOCK_PACKAGES: SeasonPackage[] = [
  ...
  },
  {
    ...
    name: 'Half Season',
    gameCount: 41,
    description: '41 selected home games',
  },
  {
    id: 'weekend',
    name: 'Weekend Plan',
    tier: 'Popular',
    gameCount: 28,
    description: 'Friday, Saturday & Sunday games',
    gameFilter: { weekendsOnly: true },
  },
  {
    id: 'friday',
    name: 'Friday Night Plan',
    gameCount: 12,
    description: 'All Friday night home games',
    gameFilter: { fridays: true },
  },
];
...
const MOCK_SECTIONS: StadiumSection[] = [
  {
    ...
    name: '101',
    level: 'Field Level',
    tags: ['Behind home plate', 'Premium'],
    rowCount: 20,
  },
  {
    id: 'sec-203',
    ...
    <div ref={ref}>
      ...
      <div
        className={`min-h-[48px] px-3 py-2 rounded-lg border-[1.5px] bg-white cursor-text flex flex-wrap items-center gap-1.5 transition-all ${
          dropdownOpen
            ? 'border-[#2c2a2b] ring-[3px] ring-[#2c2a2b]/10'
            : 'border-[#eceae5] hover:border-[#b5b1ab]'
        }`}
        onClick={() => {
      ...
      >
        ...
        <input
          ...
          placeholder={
            selectedSeats.size === 0 ? 'Type a seat number\u2026' : ''
          }
      ...
      {dropdownOpen && search.length > 0 && (
        <div className="mt-1 rounded-lg border border-[#eceae5] bg-white shadow-[0_4px_16px_rgba(0,0,0,0.10)] max-h-[180px] overflow-y-auto">
          {filtered.length === 0 ? (
            <div className="px-3.5 py-3 text-sm text-[#8e8985]">
              No matching seats
            </div>
          ) : (
            filtered.map((num, i) => (
    ...
    >
      ...
      >
        <span
          ...
          style={{ backgroundColor: isAvail ? '#d4a017' : '#2c2a2b' }}
        />
...
export default function NewPackagePage() {
  ...
  const [packages, setPackages] = useState<SeasonPackage[]>(
    DESIGN ? MOCK_PACKAGES : []
  );
  const [selectedPackage, setSelectedPackage] = useState<SeasonPackage | null>(
    DESIGN ? MOCK_PACKAGES[0] : null
  );
  ...
  function handleLeagueSelect(value: string) {
    ...
    if (!l.available) {
      showToastMsg(
        `Coming soon! We'll notify you when ${l.label} is available.`
      );
      setLeague('');
      return;
    }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      const res = await fetch(
        `/api/packages/check-slug?slug=${encodeURIComponent(linkSlug)}`
      );
      if (res.ok) {
        const data = await res.json();
  ...
  async function createPackage() {
    ...
    try {
      const gameOverrides: Record<
        string,
        { status: string; pricePerTicket: number }
      > = {};
      ...
      const seatsStr = Array.from(selectedSeats)
        .sort((a, b) => a - b)
        .join(', ');
        ...
        <div className="max-w-[480px] mx-auto px-5 pt-6 pb-12 md:py-16">
          ...
          <div className="text-center mb-4">
            ...
            <p className="text-sm text-[#8e8985] leading-relaxed hidden md:block">
              Here&apos;s what people will see when you share your link. Head to
              My season to customize games and prices first.
            </p>
          </div>

          {/* Preview — what friends will see */}
          <div className="mb-4">
            <p className="text-xs font-medium text-[#8e8985] text-center mb-4">
              Preview
            </p>
            ...
            <div className="rounded-xl border border-[#dcd7d4] overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.04)] bg-[#fefefe]">
              ...
              {(() => {
                ...
                return (
                  <div
                    className="h-[48px] flex items-center justify-between px-3 shadow-[0_1px_3px_rgba(0,0,0,0.06)]"
              ...
              {(() => {
                ...
                return (
                  <div className="px-3 pt-3 pb-2">
                    <div
                      className="flex items-center gap-2 h-9 pl-2 pr-2.5 rounded-lg"
                      style={{
                        border: `1px solid ${teamPrimary}30`,
                        backgroundColor: `${teamPrimary}08`,
                      }}
                    >
                      <div
                      ...
                      <span className="text-xs font-medium text-[#2c2a2b]">
                        Sec {selectedSection?.name || '203'} &middot; Row{' '}
                        {row || '5'} &middot; Seats{' '}
              ...
              <div className="px-3 pb-3">
                ...
                {schedule.length > 0 && (
                  <div className="flex items-center gap-1.5 mb-2.5">
                    {(() => {
                      const { accent: teamAccent } = getTeamColors(
                        selectedTeam?.name || 'San Diego Padres'
                      );
                      return (
                        <div
                    ...
                    <span className="text-sm font-semibold text-black">
                      {(() => {
                        const d = new Date(schedule[0].date);
                        return `${MONTH_NAMES[d.getMonth()]} ${d.getFullYear()}`;
                      })()}
                    </span>
                    <span className="text-[11px] font-medium text-[#8e8985]">
                      &bull; {schedule.length} games
                    </span>
                  </div>
                )}
                ...
                <div className="flex flex-col gap-1.5">
                  {schedule.slice(0, 3).map((game, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className="rounded-lg px-3 py-3 border border-[#dcd7d4] bg-white shadow-[0_2px_4px_rgba(0,0,0,0.08)] flex items-center gap-2"
                      >
                        <div className="flex items-center gap-2 shrink-0">
                          <div className="text-center w-[24px] flex flex-col items-center gap-px">
                            <div className="text-[10px] font-medium text-[#8e8985] uppercase">
                              {dow}
                            </div>
                            <div className="text-sm font-extrabold text-[#2c2a2b] leading-tight">
                              {day}
                            </div>
                            <div className="text-[10px] font-medium text-[#8e8985]">
                              {mon}
                            </div>
                          </div>
                          <div className="w-px h-[40px] bg-[#dcd7d4]" />
                          <div
                            className="w-[28px] h-[28px] rounded-full flex items-center justify-center text-[8px] font-bold text-white shrink-0"
                            style={{ backgroundColor: color }}
                          >
                            {abbr}
                          </div>
                        ...
                        <div className="flex flex-col gap-0.5 min-w-0 flex-1">
                          <div className="text-sm font-bold text-[#2c2a2b]">
                            vs {shortName}
                          </div>
                          <div className="text-xs font-medium text-[#8e8985]">
                            {game.time} &bull; {selectedTeam?.venue || 'the venue'}
                          </div>
                        </div>
                        <svg
                          width="16"
          ...
          <div className="rounded-lg border border-[#eceae5] bg-white px-4 py-3 mb-6 flex items-center gap-2">
            <span className="text-sm text-[#2c2a2b] flex-1 text-left truncate">
              benchbuddy.com/share/{result.shareLink || linkSlug}
            </span>
            <button
              onClick={() => {
                navigator.clipboard.writeText(
                  `${window.location.origin}/share/${result.shareLink || linkSlug}`
                );
                setCopied(true);
    ...
    <SetupLayout steps={STEPS} currentStep={step} showSidebar={false}>
        ...
        {STEPS.map((s, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-2">
                ...
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${
                    isDone
                      ...
                      : isActive
                        ? 'bg-[#2c2a2b] text-white'
                        : 'bg-[#eceae5] text-[#b0a89e]'
                  }`}
                >
                  {isDone ? (
              ...
              {i < STEPS.length - 1 && (
                <div
                  className={`w-6 h-[2px] ${isDone ? 'bg-[#8B2500]' : 'bg-[#eceae5]'}`}
                />
              )}
            </div>
          );
      ...
      {step === 1 && (
        <div className="flex flex-col flex-1">
          <StepHeadline>
            <span className="md:hidden">Your tickets</span>
            <span className="hidden md:inline">Which tickets do you have?</span>
          </StepHeadline>
          <StepSubhead>
            We&apos;ll pull in your game schedule based on what you select here.
          </StepSubhead>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <FormSelect
                ...
                onChange={(v) => {
                  const t = teams.find((t) => t.id === parseInt(v));
          ...
          <div>
            ...
            <FormSelect
              ...
              onChange={(v) => {
                const p = packages.find((p) => p.id === v);
                setSelectedPackage(p || null);
              }}
              placeholder="Select your season tickets..."
            >
              {packages.map((p) => (
          ...
          {selectedPackage && (
            <div className="rounded-lg bg-[#FEF3CD] text-[#856D10] px-4 py-3 text-sm font-medium mt-4 leading-relaxed">
              ⚠️ We&apos;ll load your games based on this selection. Make sure
              it matches your season tickets — you can remove games but
              can&apos;t add new ones.
            </div>
          )}
          ...
          <StepActions>
            <PrimaryButton
              ...
              disabled={!league || !selectedTeam || !selectedPackage}
            >
              Continue →
            </PrimaryButton>
          </StepActions>
      ...
      {step === 2 && (
        <div className="flex flex-col flex-1">
          <StepHeadline>
            <span className="md:hidden">Your seats</span>
            <span className="hidden md:inline">Where do you sit?</span>
          </StepHeadline>
          <StepSubhead>
            We&apos;ll show this to anyone you share with so they know the seats
            they&apos;re getting.
          </StepSubhead>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <FormSelect
                ...
                onChange={(v) => {
                  const s = sections.find((s) => s.id === v);
              ...
              return (
                ...
                >
                  ...
                  <span className="text-sm text-white">
                    <strong>{selectedTeam?.name || 'San Diego Padres'}</strong>{' '}
            ...
            <div className="bg-white divide-y divide-[#f5f4f2]">
              <div className="flex items-center justify-between px-5 py-3.5">
                ...
                <span className="text-sm font-bold text-[#2c2a2b]">
                  {selectedPackage?.name || 'Full Season'} &middot;{' '}
                  {selectedPackage?.gameCount || 81} Games
                </span>
              </div>
              <div className="flex items-center justify-between px-5 py-3.5">
                <span className="text-sm text-[#8e8985]">
                  Section &amp; Row
                </span>
                <span className="text-sm font-bold text-[#2c2a2b]">
                  Sec {selectedSection?.name || '203'} &middot; Row {row || '5'}
                </span>
              </div>
              <div className="flex items-center justify-between px-5 py-3.5">
